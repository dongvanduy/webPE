    ...
    const Import = {
        handleTrayInput: () => {
            ...
            trayInput.addEventListener("input", function () {
                ...
                const patternCarton = /^CARTON-(\d+)$/;
                const patternWithoutLevel = /^([A-Z]+)(\d+)-K(\d+)$/;
                const locationPattern = /^([A-Z]+)(\d+)-(\d+)-K(\d+)$/;

                const matchCarton = patternCarton.exec(location);
        ...
        handleSerialInput: () => {
            ...
            serialInput.addEventListener("input", async function () {
                ...
                document.getElementById("serial-limit-warning").style.display = totalSerials > 600 ? "block" : "none";
                document.getElementById("serial-count").textContent = `Tổng số serial đã nhập: ${totalSerials}/600`;
